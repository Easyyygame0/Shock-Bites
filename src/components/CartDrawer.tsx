    ...
    <>
      ...
      <div className={`cart-drawer ${cartOpen ? "open" : ""}`}>
        ...
        <div className="p-4 sm:p-6 flex items-center justify-between flex-shrink-0" style={{ background: "#1e5c2e" }}>
          <span className="font-display text-xl sm:text-2xl tracking-widest text-white">YOUR ORDER</span>
          <button
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          {cart.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-4 py-16 sm:py-20" style={{ color: "#5a7a4a" }}>
              <svg className="w-14 h-14 sm:w-16 sm:h-16 opacity-20" fill="none" stroke="currentColor" strokeWidth="1.5" viewBox="0 0 24 24">
                <path d="M6 2 3 6v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V6l-3-4z"/>
                <line x1="3" y1="6" x2="21" y2="6"/>
                <path d="M16 10a4 4 0 0 1-8 0"/>
              </svg>
              <p className="font-display text-lg sm:text-xl tracking-wider text-center" style={{ color: "rgba(30,92,46,0.35)" }}>
                Your bag is shockingly empty
              </p>
            ...
            <div className="space-y-3 sm:space-y-4">
              {cart.map(item => {
                ...
                return (
                  <div
                    key={item.product.id}
                    className="flex gap-3 sm:gap-4 items-center p-3 sm:p-4 rounded-xl border"
                    style={{ background: "#f0f4e8", borderColor: "rgba(30,92,46,0.15)" }}
                  >
                    {item.product.image ? (
                      <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-xl overflow-hidden flex-shrink-0 bg-white shadow-sm">
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-bold text-xs sm:text-sm leading-tight truncate" style={{ color: "#1a3a1a" }}>{item.product.name}</h4>
                    ...
                    <div className="text-right flex flex-col items-end gap-1 sm:gap-2">
                      <span className="font-black text-sm sm:text-base" style={{ color: "#1a3a1a" }}>₱{item.product.price * item.quantity}</span>
        ...
        {/* Footer */}
        {cart.length > 0 && (
          <div className="p-4 sm:p-6 border-t flex-shrink-0 space-y-3 sm:space-y-4"
               style={{ background: "#f0f4e8", borderColor: "rgba(30,92,46,0.15)" }}>
            <div className="flex justify-between items-center">
              <span className="font-display text-lg sm:text-xl tracking-wider" style={{ color: "#1a3a1a" }}>SUBTOTAL</span>
              <span className="font-black text-xl sm:text-2xl" style={{ color: "#1e5c2e" }}>₱{subtotal}</span>
            </div>
            <button
              onClick={openCheckout}
              className="w-full py-3 sm:py-4 rounded-xl font-display tracking-widest text-base sm:text-lg text-white shadow-md hover:opacity-90 transition-all"
              style={{ background: "#1e5c2e" }}
            >
              PROCEED TO CHECKOUT
            </button>
